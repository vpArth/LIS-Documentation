...
### HTTP методы ЛИС.
    ...
    - Метод приёма заказов. `/putOrders`
    - Метод отмены заказа. `/cancelOrder`
...
### HTTP методы МИС.
    ...
    Вызываемый метод определяется GET параметром `action` (настраиваемые)
        - Метод приёма результата `action=get`
...
Для успешной интеграции, ЛИС должна предоставить специалистам по МИС следующие данные:
 ...
 - `/api/putOrders` - точка входа в API для передачи заказов
 - `/api/cancelOrder` - точка входа в API для отмены заказа
